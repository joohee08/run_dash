...
class Ep1 extends Phaser.Scene {
	...
	editorCreate() {
		...
		// pg1back
		const pg1back = this.add.image(635, 360, "pg1back");
		pg1back.scaleX = 0.67;
		pg1back.scaleY = 0.67;
		...
		// pg1flor
		const pg1flor = this.add.image(0, 1.030029296875, "pg1flor");
		pgfloors.add(pg1flor);

		// pg1flor_1
		const pg1flor_1 = this.add.image(299.1600036621094, 0.9200439453125, "pg1flor");
		pgfloors.add(pg1flor_1);

		// pg1flor_2
		const pg1flor_2 = this.add.image(595.260009765625, 0.9100341796875, "pg1flor");
		pgfloors.add(pg1flor_2);

		// pg1flor_3
		const pg1flor_3 = this.add.image(891.8199462890625, 0.6600341796875, "pg1flor");
		pgfloors.add(pg1flor_3);
	...
	Ep1shrinkPlayer() {
		...
		const bodyWidth = this.player.width * newScale * 0.55;
		const bodyHeight = this.player.height * newScale * 6.5; 
		this.player.body.setSize(bodyWidth, bodyHeight);

		this.player.body.setOffset(
			(this.player.width * newScale - bodyWidth) / 2,
			this.player.height * newScale * 0.8   
		);
	}
	...
	Ep1restorePlayer() {
		...
		const bodyWidth = this.player.width * newScale * 0.55;
		const bodyHeight = this.player.height * newScale * 3.8;

		this.player.body.setSize(bodyWidth, bodyHeight);

		this.player.body.setOffset(
			(this.player.width * newScale - bodyWidth) / 2,
			this.player.height * newScale * 0.55
		);
	}
